...
function clearAll(){
    let wantClear = false;
    wantClear = confirm('All data from "Work Day Scheduler" will be deleted');
   if (wantClear){
        localStorage.removeItem("taskArray");
        localStorage.removeItem("hoursFrom");
        localStorage.removeItem("hoursTotal");
        for (i = 0; i < timeFormElements.children().length; i++) {
            timeFormElements.children().eq(i).children().eq(1).val("");
        }
